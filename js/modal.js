...
// Esto crea la estructura oculta del pop-up al final del body automáticamente.
document.addEventListener('DOMContentLoaded', () => {
    const modalHTML = `
        <div id="custom-modal-overlay" class="fixed inset-0 bg-black bg-opacity-60 z-[100] hidden flex items-center justify-center p-4 backdrop-blur-sm transition-opacity duration-300 opacity-0">
            <div id="custom-modal-box" class="bg-white w-full max-w-md rounded-lg shadow-2xl overflow-hidden scale-95 transition-transform duration-300 border-t-4 border-rose-600">
                <div class="flex justify-between items-center p-4 border-b border-gray-100 bg-gray-50">
                    ...
                    <button onclick="closeModal()" class="text-gray-400 hover:text-rose-600 transition-colors">
                ...
                <div id="modal-actions" class="p-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                    <button id="btn-modal-cancel" onclick="closeModal()" class="hidden px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded text-sm font-bold uppercase hover:bg-gray-100 transition-colors">
                        Cancelar
                    </button>
    ...
    // Cerrar si se hace clic fuera del modal (en el fondo oscuro)
...
// type puede ser 'alert' (solo aviso) o 'confirm' (pregunta sí/no)
window.showAlert = function(title, message, type = 'alert', onConfirm = null) {
    ...
    if (type === 'confirm') {
        ...
        confirmBtn.classList.add('bg-red-700', 'hover:bg-red-800'); // Botón rojo para peligro
        currentConfirmAction = onConfirm; // Guardamos qué función ejecutar si dicen SÍ
    } else {
        cancelBtn.classList.add('hidden');
        confirmBtn.innerText = 'ACEPTAR';
        confirmBtn.classList.remove('bg-red-700', 'hover:bg-red-800');
        confirmBtn.classList.add('bg-rose-600', 'hover:bg-rose-700'); // Botón rosa normal
        currentConfirmAction = closeModal; // Si es alerta, aceptar solo cierra
    }
